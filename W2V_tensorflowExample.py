...
class DWE(Model):
    def __init__(self,v,d,T):
        ...
        self.U = layers.Embedding(v,d,embeddings_initializer = RandomUniform(minval=-2, maxval=2))
        self.V = layers.Embedding(v,d,embeddings_initializer = RandomUniform(minval=-2, maxval=2))
...
print("Vocabulary of size %d with %d observed cooccurences" % (len(voc),np.sum(cooc)))
